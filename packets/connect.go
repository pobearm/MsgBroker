...
func (c *ConnectPacket) Validate(username, password string) byte {
	...
	//不允许客户端id为空
	if len(c.ClientIdentifier) > 65535 || len(c.ClientIdentifier) == 0 ||
		len(c.Username) > 65535 || len(c.Password) > 65535 {
		fmt.Println("Bad size field")
		return CONN_PROTOCOL_VIOLATION
	}

	if username != c.Username || password != string(c.Password) {
