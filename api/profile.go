...
// debug=0 prints only the hexadecimal addresses that pprof needs.
// debug=1 adds comments translating addresses to function names
// 		and line numbers, so that a programmer can read the profile without tools.
// debug=2 means to print the goroutine stacks in the same form that a Go program uses
// 		when dying due to an unrecovered panic.
func ProcessInput(input string, debug int, w io.Writer) {
	switch input {
	case "goroutine":
